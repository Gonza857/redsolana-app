...
import { AdminBar } from "../../../components/Admin/Checkers/AdminBar.jsx";
...
export function Checkers() {
  ...
  return (
    <AdminCajerosContainer className="col-12 py-2">
      <Wrapper className="col-12 col-sm-10 col-lg-9 col-xl-6">
        <AdminBar />
        {isSearchingCajero ? (
          <>
          ...
          {isSearchingCajero ? (
            <>
              {searchResult.map((cajero) => {
                return <CajeroAdmin cajero={cajero} key={cajero._id} />;
              })}
            </>
          ) : (
            <>
