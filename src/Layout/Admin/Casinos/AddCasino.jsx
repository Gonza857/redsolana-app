...
import Casino from "../../../classes/Casino";

export const AddCasino = () => {
  ...
  const { casinos, c_addCasino } = useContext(adminContext);

  const { resetField } = useForm();

  const uploadCasino = () => {
    let a = new Casino(null, casinoName, casinoLink);
    let c = solana.createCasino({ ...a });
    if (!c._link.includes("https://")) {
      c._link = `https://${c.link}`;
    }
    c_addCasino(c, previewImageUrl);
  };
  ...
  return (
    <div className="col-12 d-flex flex-column align-items-center py-4 py-xl-2">
      ...
      <div className="col-11 col-xl-12 d-flex d-flex flex-wrap justify-content-center">
        ...
        >
          <StyledForm className="col-12 col-sm-10 col-md-8 col-lg-7 col-xl-8 d-flex flex-column gap-3 p-3 p-xl-3">
            <InputContainer>
